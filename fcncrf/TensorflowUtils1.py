...
def bilinear_weight_variable(factor, number_of_classes, name=None):
    initial = bilinear_upsample_weights(factor, number_of_classes)
    if name is None:
        return tf.Variable(initial)
    else:
        return tf.get_variable(name, initializer=initial)   
...
#     return normed
def batch_norm(x, is_traning):
    """
    Code taken from http://stackoverflow.com/a/34634291/2267819
    """
    normed = tf.layers.batch_normalization(x, training = is_traning)
    return normed
